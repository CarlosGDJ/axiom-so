...
interface CheckinDoc { id: string; fecha: string; agreement: number }
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) { setOpen(false); setDismissed(true); } }}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogFooter className="flex-col sm:flex-row gap-2">
          <Button variant="ghost" onClick={() => { setOpen(false); setDismissed(true); }}>Ahora no</Button>
          <Button onClick={handleSave} disabled={agreement == null}>Guardar cierre</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
